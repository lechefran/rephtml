...
func TestInlineComponentOutputFormats(t *testing.T) {
	tests := []struct {
		...
		{name: "Dbi", element: NewDbi().Text("Isolate"), want: `<dbi>Isolate</dbi>`},
		{name: "Dbo", element: NewDbo().Text("Override"), want: `<dbo>Override</dbo>`},
		...
		{name: "Elem", element: NewElem().Text("Emphasis"), want: `<elem>Emphasis</elem>`},
